...
.result_box1 {margin-left:28%;width:850px;color:#555;font-size:20pt;font-family:Verdana;text-align:left;border:0px solid red;}  
.result_boxpic {margin:0 auto;width:700px;height:352px;padding:100px 0;text-align:center;border:0px solid #40AA53;background:#efe;background:url(jiangnew.jpg) center 0 no-repeat;}  
...
.result_lottery{margin:146px 0px 5px 0px; color:white; font-size:28px;font-weight: bold;  text-align:center; font-family:"microsoft yahei"; text-shadow:3px 3px 2px rgba(0,0,0,0.2)}
.result_download{margin:100px 0px 5px 0px; color:#555; font-size:18px;text-align:center; font-family:"microsoft yahei"; text-shadow:2px 2px 2px rgba(0,0,0,0.2)}

.result_clock{ margin:-60px 20px 0px 20px;  position:absolute; color:white; font-size:22px; font-weight: bold; text-align:center; font-family:"microsoft yahei"; text-shadow:3px 3px 2px rgba(0,0,0,0.2);border:0px solid red;}
...
SELECT ord_id,ord_mct_id,mct_name,tbl_order.ord_pmt_name,tbl_order.ord_trade_amount/100, tbl_order.ord_trade_time,tbl_order.ord_status FROM tbl_order  left join tbl_merchant on  tbl_order.ord_mct_no=tbl_merchant.mct_no where  ord_trade_time>='2017-07-20' and ord_status=1 and ord_trade_amount>=1000 order by rand() limit 3000";
 ...
 $sql_str="SELECT ord_no,ord_mct_no,mct_name,ord_pmt_name,ord_trade_amount/100, ord_trade_time,ord_status,tbl_agent.agent_name FROM ((select * from tbl_order_copy2 where ord_trade_time>='2017-07-20' and ord_status=1 and ord_trade_amount>=1000 ) as tbl_order_copy  left join tbl_merchant on  tbl_order_copy.ord_mct_no=tbl_merchant.mct_no) left join tbl_agent on tbl_order_copy.ord_agent_no=tbl_agent.agent_no order by rand() limit 3000";
...
//alldata = "遂川县泉江镇洋货铺子商行 微信支付 268元 2017-08-06 09:01:10 吉安分行遂川支行 订单号:860000003922948,遂川县泉江镇洋货铺子商行 微信支付 268元 2017-08-06 09:01:10 吉安分行遂川支行 订单号:22333333333,遂川县泉江镇洋货铺子商行 微信支付 268元 2017-08-06 09:01:10 吉安分行遂川支行 订单号:23333333333333,于都雨辰油漆商行 微信支付 2000元 2017-07-11 16:51:19 小企业信贷中心 订单号:860000003662308";
...
function ok(){   
    ...
    document.getElementById("resultDiv").innerHTML = document.getElementById("resultDiv").innerHTML +" <br>"+ "<font  size='4' >"+document.getElementById("oknum").value +"</font>";
    stopflag=1; 
      clicknum=clicknum+1;
    ...
    try{
        ...
        for(i=0;i<alldataarr.length;i++){
        	...
        	for(j=1;j<=clicknum;j++){
        	    var lotteryMerchantName=resultArray[j].split(" ")[0];
       ...
       if(clicknum>=2){
	      ...
	      document.getElementById("endnameDiv").innerHTML=resultArray[1].split(" ")[0] +"<br><font size='3'>("+resultArray[1].split(" ")[5] +")</font><br>"+resultArray[2].split(" ")[0] +"<br><font size='3'>("+resultArray[2].split(" ")[5]+")</font>";
...
<div class="login_infonew" >赣州银行商户通抽奖(第三期)</div>
...
<ul>
	<li><b>活动说明：</b></li>
<li>1. 本次抽奖活动连续开展3期（即7月20日-8月20日、8月21日-9月20日、9月21日-10月20日）</li>
<li>2. 活动对象为赣州银行商户通签约商户（已开立赣州银行结算账户），且每期金额10元以上（含）的交易笔数大等于10笔</li>
<li>3. 活动随机抽取期内两笔10元以上（含）交易，交易归属商户各获赠iPhone 7 Plus 1台 </li>
<li>4. 同一商户每期只可抽中一次</li>
</ul>
</div>
</body>  
